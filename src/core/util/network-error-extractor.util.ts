...
import { ERROR_SOURCE_DIFE, ERROR_SOURCE_MOL } from './error-constants.util';
...
export function extractNetworkErrorInfo(errorMessage: string): NetworkErrorInfo | null {
  if (!errorMessage) {
    return null;
  }
...
function extractDifeError(errorMessage: string): NetworkErrorInfo | null {
...
function extractMolErrorCode(errorMessage: string): NetworkErrorInfo | null {
  ...
  if (molErrorCodeMatch) {
    return {
      ...
      source: ERROR_SOURCE_MOL
    };
  }
  return null;
}

function extractMolError(errorMessage: string): NetworkErrorInfo | null {
...
function extractMolCodeOnly(errorMessage: string): NetworkErrorInfo | null {
...
function extractExternalServiceError(errorMessage: string): NetworkErrorInfo | null {
...
function extractHttpStatusError(errorMessage: string): NetworkErrorInfo | null {
...
function extractJsonError(errorMessage: string): NetworkErrorInfo | null {
...
function extractErrorByKeywords(errorMessage: string): NetworkErrorInfo | null {
...
function determineSourceFromMessage(errorMessage: string): 'DIFE' | 'MOL' {
  const lowerMessage = errorMessage.toLowerCase();
